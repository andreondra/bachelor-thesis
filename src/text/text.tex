...
PPU je řízeno pomocí osmi registrů mapovaných do paměťového prostoru procesoru. Tyto registry jsou zrcadleny po širokém adresním rozsahu, jak bylo uvedeno v~úvodu. Tabulka~\ref{tab:ppu-registry} poskytuje přehled a~stručný popis. Pro podrobnější informace je možné čtenáře opět odkázat na:  DOPLNIT.
...
\begin{table}[ht!]
	...
	\begin{tblr}{|Q[c,m]|Q[c,m]|Q[c,m]|X[l,m]|}
		\hline
		Označení & Adresa & Bity & Popis bitů \\
		\hline[2pt]
		\SetCell[r=7]{c} PPUCTRL & \SetCell[r=7]{c} \$2000 & \SetCell[r=7]{c} \texttt{VPHB SINN} & \texttt{V}: Povolení generování NMI \\ \cline{4} & & & \texttt{P}: režim PPU (0: slave, 1: master) \\ \cline{4} & & & \texttt{H}: Velikost sprajtů (0: $8\times8$, 1: $8\times16$) \\ \cline{4} & & & \texttt{B}: Výběr pattern table pro pozadí (0: první, 1: druhá) \\ \cline{4} & & & \texttt{S}: Výběr pattern table pro sprajty (0: první, 1: druhá) \\ \cline{4} & & & \texttt{I}: Způsob inkrementace indexu do videopaměti (0: přičíst 1, 1: přičíst 32) \\ \cline{4} & & & \texttt{NN}: Základ pro index do videopaměti (0: \$2000, 1: \$2400, 2: \$2800, 3: \$2C00) \\
		\hline
		\SetCell[r=6]{c} PPUMASK & \SetCell[r=6]{c} \$2001 & \SetCell[r=6]{c} \texttt{BGRs bMmG} & \texttt{BGR}: Zvýraznění barev (jednobitová hodnota pro modrou, zelenou a~červenou, 1 zvýraznění aktivuje) \\ \cline{4} & & & \texttt{s}: Povolení sprajtů \\ \cline{4} & & & \texttt{b}: Povolení pozadí \\ \cline{4} & & & \texttt{M}: 1: zobrazit sprajty v~levých 8~pixelech obrazovky, 0: skrýt \\ \cline{4} & & & \texttt{m}: 1: zobrazit pozadí v~levých 8~pixelech obrazovky, 0: skrýt \\ \cline{4} & & & \texttt{G}: 1: renderovat ve stupních šedi, 0: renderovat barevně \\
		\hline		
	\end{tblr}
\end{table}
