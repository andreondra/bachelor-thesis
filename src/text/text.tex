...
\subsection{Systémy}
Po vyvinutí komponent je možné definovat jejich propojení v~rámci systému, jehož obecné rozhraní zajišťuje třída \mintinline{cpp}|System|. Systém obsahuje metody pro postupování emulací po různě velkých úsecích, například po jednotlivých taktech hlavních hodin lze postupovat pomocí \mintinline{cpp}|doClocks()|. Je zajištěno~i~rozhraní pro běh v~reálném čase: \mintinline{cpp}|doRun()|.

Systém obsahuje mnoho předpřipravených metod. Týká se to především práce se společným rozhraním komponent. Přidá-li programátor používané komponenty do připraveného kontejneru typu vektor s~identifikátorem \mintinline{cpp}|m_components|, je možné tyto metody použít. Jsou to metody pro získání definic grafického rozhraní všech komponent (\mintinline{cpp}|getGUIs()|), získání všech zdrojů zvuku (\mintinline{cpp}|getSampleSources()|), definice všech dostupných uživatelských vstupů (\mintinline{cpp}|getInputs()|), popřípadě \mintinline{cpp}|init()| pro skupinovou inicializaci všech komponent.
